...
def caihongpi():
    if (Whether_caihongpi != False):
        try:
            ...
            if ("XXX" in data):
                data=data.replace("XXX", "仙女妹妹")
            return data
        except:
            return ("彩虹屁API调取错误，请检查API是否正确申请或是否填写正确")

def tuweiqinghua():
    if (Whether_tuweiqinghua != False):
        try:
            key = config["tian_api"]
            conn = http.client.HTTPSConnection('api.tianapi.com')  # 接口域名
            params = urllib.parse.urlencode({'key': key})
...
def send_message(to_user, access_token, region_name, weather, temp, wind_dir, note_ch, note_en, max_temp, min_temp,
                 sunrise, sunset, category, pm2p5, proposal, chp, pipi, twqh):
...
if __name__ == "__main__":
    ...
    Whether_tuweiqinghua = config["Whether_tuweiqinghua"]
    ...
    for user in users:
        send_message(user, accessToken, region, weather, temp, wind_dir, note_ch, note_en, max_temp, min_temp, sunrise,
                     sunset, category, pm2p5, proposal, chp, pipi, twqh)
    os.system("pause")
